...
describe('SSRF Protection', () => {
  ...
  describe('validateActionDomain', () => {
    ...
    it('validates matching IP addresses when client provides just IP (no protocol)', () => {
      const result = validateActionDomain('10.0.0.5', 'http://10.0.0.5:7894/api');
      expect(result.isValid).toBe(true);
      expect(result.normalizedSpecDomain).toBe('http://10.0.0.5');
      expect(result.normalizedClientDomain).toBe('http://10.0.0.5');
    });
    ...
    it('validates matching IP when client provides full URL with IP', () => {
      const result = validateActionDomain('http://10.0.0.5', 'http://10.0.0.5:7894');
      expect(result.isValid).toBe(true);
      expect(result.normalizedSpecDomain).toBe('http://10.0.0.5');
      expect(result.normalizedClientDomain).toBe('http://10.0.0.5');
    });
    ...
    it('rejects mismatched IP addresses', () => {
      const result = validateActionDomain('10.0.0.5', 'http://10.0.0.6:7894/api');
      expect(result.isValid).toBe(false);
      expect(result.message).toContain('Domain mismatch');
      expect(result.message).toContain('10.0.0.5');
      expect(result.message).toContain('10.0.0.6');
    });
