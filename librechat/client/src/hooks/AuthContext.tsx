...
import { useMkSsoCallback } from './MkSso';
...
const AuthContextProvider = ({
  ...
  // MK SSO 集成
  const mkSsoEnabled = import.meta.env.VITE_MK_SSO_ENABLED === 'true';
  const mkSsoLoginUrl = import.meta.env.VITE_MK_SSO_LOGIN_URL;
  const searchParams = useMemo(() => new URLSearchParams(window.location.search), []);
  const hasMkCode = searchParams.has('mkCode');

  useMkSsoCallback({ enabled: hasMkCode });

  // MK SSO 重定向：只有当启用、未认证、没有 mkCode 时才重定向
  const shouldRedirectToSso = mkSsoEnabled && !isAuthenticated && !hasMkCode && !!mkSsoLoginUrl;

  // ===== DEBUG: SSO Redirect Investigation =====
  console.group('🔍 MK SSO Configuration & Redirect Logic');
  console.log('Environment Variables:');
  console.log('  VITE_MK_SSO_ENABLED:', import.meta.env.VITE_MK_SSO_ENABLED);
  console.log('  VITE_MK_SSO_LOGIN_URL:', import.meta.env.VITE_MK_SSO_LOGIN_URL);
  console.log('  VITE_MK_SSO_APP_ID:', import.meta.env.VITE_MK_SSO_APP_ID);
  console.log('\nSSO Redirect Conditions:');
  console.log('  mkSsoEnabled:', mkSsoEnabled);
  console.log('  isAuthenticated:', isAuthenticated);
  console.log('  hasMkCode:', hasMkCode);
  console.log('  mkSsoLoginUrl:', mkSsoLoginUrl);
  console.log('\nFinal Decision:');
  console.log('  shouldRedirectToSso:', shouldRedirectToSso);
  if (shouldRedirectToSso) {
    console.log('  ✅ WILL redirect to SSO:', mkSsoLoginUrl);
  } else {
    console.log('  ❌ WILL NOT redirect. Reasons:');
    if (!mkSsoEnabled) console.log('    - SSO is disabled (VITE_MK_SSO_ENABLED is not "true")');
    if (isAuthenticated) console.log('    - User is already authenticated');
    if (hasMkCode) console.log('    - mkCode parameter present in URL');
    if (!mkSsoLoginUrl) console.log('    - mkSsoLoginUrl is empty or undefined');
  }
  console.groupEnd();
  // ===== END DEBUG =====
  ...
  useEffect(() => {
    ...
    if (token == null || !token || !isAuthenticated) {
      // ===== DEBUG: SSO Redirect Execution =====
      console.group('🔄 SSO Redirect Execution Check');
      ...
      // MK SSO 重定向：在 silentRefresh 之前检查是否需要跳转到 SSO 登录
      if (shouldRedirectToSso) {
        // 构建完整的SSO重定向URL，包含appId和redirection_url参数
        const appId = import.meta.env.VITE_MK_SSO_APP_ID;
        const currentUrl = window.location.origin + window.location.pathname;
        const ssoUrl = `${mkSsoLoginUrl}?appId=${appId}&redirection_url=${encodeURIComponent(currentUrl)}`;

        console.log('🚀 Executing SSO redirect to:', ssoUrl);
        console.groupEnd();
        window.location.href = ssoUrl;
        return;
      }

      // 如果有 mkCode，useMkSsoCallback 正在处理，跳过 silentRefresh
      if (hasMkCode) {
        console.log('⏭️ Skipping silentRefresh, mkCode processing in progress');
        console.groupEnd();
        return;
      }

      console.log('📞 Calling silentRefresh()');
      console.groupEnd();
      // ===== END DEBUG =====
